...
손으로 쓰는 비밀 일기장 — AI가 읽고, 기억하고, 손글씨로 답장해주는 감성 다이어리 앱
...
- 🎨 **감성 디자인** — 가죽 위 낡은 종이 묶음, 언어별 무료 손글씨 폰트, 획별로 번지며 스며드는 잉크, 가죽 일기장이 펼쳐지는 오프닝
- 🗓️ **캘린더·메모·상담·아이디어 (POC)** — 달력에서 기록을 한눈에, AI 없는 조용한 메모, 깊이 들어주는 상담 모드, 스케치를 그리면 AI가 그림을 보고 해석·구조화해주는 아이디어 모드
...
손글씨 대신 **키보드 채팅**, 감성 대신 **직설**. ENTP 팩폭 코치가
진단 → 핑계 차단 → 선택 강요 → 행동 명령(시간 포함) → 검증 예고 순서로 답한다.
